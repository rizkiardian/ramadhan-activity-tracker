...
@section('title', 'Tambah Aktivitas — Ramadhan Activity Tracker')
@section('page-title', 'Tambah Aktivitas')
@section('page-subtitle', 'Catat aktivitas ibadah baru')
    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
      <form method="POST" action="{{ route('activities.store') }}" class="space-y-5">
        ...
        <div>
          ...
          <input id="date" type="date" name="date" value="{{ old('date', now()->format('Y-m-d')) }}"
        ...
        <div>
          ...
          <div class="grid grid-cols-3 gap-3">
            @foreach (['Pending' => 'Pending', 'Done' => 'Selesai', 'Skipped' => 'Dilewati'] as $value => $label)
              <label
                class="flex items-center gap-2 cursor-pointer border rounded-lg px-3 py-2.5 transition-colors {{ old('status', 'Pending') === $value ? 'border-primary-500 bg-primary-50' : 'border-gray-300 hover:border-gray-400' }}">
        ...
        <div>
          ...
          <textarea id="notes" name="notes" rows="3" placeholder="Tambahkan catatan (opsional)..."
            class="w-full text-sm border rounded-lg px-3.5 py-2.5 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 resize-none {{ $errors->has('notes') ? 'border-red-400 bg-red-50' : 'border-gray-300' }}">{{ old('notes') }}</textarea>
          @error('notes')
            <p class="mt-1.5 text-xs text-red-600">{{ $message }}</p>
          @enderror
        </div>
